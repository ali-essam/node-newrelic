...
function getActiveSegment(obj) {
  ...
  if (segment && segment.transaction) {
    if (typeof segment.transaction.isActive === 'function') {
    ...
    try {
      console.log('NEWRELICDEBUG segment', safeJsonStringify(segment))
    } catch (e) {}
    try {
      var bkupToJSON = segment.toJSON
      segment.toJSON = undefined
      console.log('NEWRELICDEBUG segment (no toJSON)', safeJsonStringify(segment))
      segment.toJSON = bkupToJSON
    } catch (e) {}
    try {
      console.log('NEWRELICDEBUG segment.transaction', safeJsonStringify(segment.transaction))
    } catch (e) {}
    try {
      console.log('NEWRELICDEBUG typeof segment.transaction', safeJsonStringify(typeof segment.transaction))
    } catch (e) {}
    try {
      console.log('NEWRELICDEBUG segment.transaction.isActive', safeJsonStringify(segment.transaction.isActive))
    } catch (e) {}
    try {
      console.log('NEWRELICDEBUG typeof segment.transaction.isActive', safeJsonStringify(typeof segment.transaction.isActive))
    } catch (e) {}
  }
  return null
}
